...
import models.xml.FileErrorCode.UnknownErrorCode
...
class FileErrorsSpec extends SpecBase {
  ...
  "FileErrors" - {
    ...
    "must read xml as FileErrors for unknown code" in {
      ...
      XmlReader.of[FileErrors].read(xml) mustBe ParseSuccess(FileErrors(UnknownErrorCode("50011"), Some("error message")))
    }
